...
def get_number_of_doors_map():
    ...
    if resp.status_code != 200:
        # This means something went wrong.
        raise RuntimeError(
            'GET ' + config.application['APPLICATION_ROOT'] + '/auto/riskestimator/features/number_of_doors {}'.format(resp.status_code))
    return resp.json()
...
def get_number_of_cylinders_map():
    ...
    if resp.status_code != 200:
        # This means something went wrong.
        raise RuntimeError(
            'GET ' + config.application['APPLICATION_ROOT'] + '/auto/riskestimator/features/number_of_cylinders {}'.format(resp.status_code))
    return resp.json()
